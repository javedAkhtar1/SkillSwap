...
export async function completeProfileController(req: Request) {
  const session = await getServerSession();
  if (!session || !session.user?.email) {
    return errorResponse("Unauthorized", 401);
  }

  const body = completeProfileSchema.parse(await req.json());
  const { age, bio, profilePicture, skillsToLearn, skillsToTeach } = body;
